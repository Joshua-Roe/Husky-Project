...
#   pose_target.position.y = 0.0
 ...
 if stage == 3:
   rospy.sleep(0.5)
   interpreter.execute("go " + "right" + " " + str(0.15))
   stage = 4
 ...
 if stage == 4:
   ...
   pose_target.orientation.z = -0.639
   pose_target.orientation.w = 0.659
   pose_target.position.x = 0.344
   pose_target.position.y = 0.354
#   pose_target.position.y = 0.5
#   pose_target.position.z = 0.972
   pose_target.position.z = 1.1
   group.set_pose_target(pose_target)

   plan1 = group.plan()
   rospy.loginfo("============ Visualizing stow")
   display_trajectory = moveit_msgs.msg.DisplayTrajectory()
 ...
 if stage == 5:
   rospy.loginfo("getting into position")
   try:
    (trans,rot) = listener.lookupTransform('/map', '/start_pos', rospy.Time(0))
   except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
    continue
   quaternion = (rot[0], rot[1], rot[2], rot[3])
#   trans[0] = 0
...
#   trans[2] = 0
 ...
 if stage == 6:
   ...
   pose_target.orientation.w = 0.659
   pose_target.position.x = atrans[0]
   pose_target.position.y = atrans[1]
   pose_target.position.z = atrans[2] - 0.57
   group.set_pose_target(pose_target)

   plan1 = group.plan()
   rospy.loginfo("============ Visualizing place")
   display_trajectory = moveit_msgs.msg.DisplayTrajectory()
